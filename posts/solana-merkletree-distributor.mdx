...
**What we're building**: An airdrop system that can distribute SOL (Solana's native token) to thousands of people efficiently and cheaply. Instead of storing a massive list of recipients on the blockchain (which would be expensive), we use a Merkle tree to store just a single 32-byte "fingerprint" that represents all recipients.
